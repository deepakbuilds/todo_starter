...
from todos.utils import error_for_list_title, find_list_by_id, error_for_todo, find_todo_by_id, delete_todo_by_id, mark_all_completed, todos_remaining, is_list_completed, is_todo_completed, sort_items
...
def update_todo_status(lst, todo, list_id, todo_id):
    ...
    flash(f"The todo has been {'checked' if todo['completed'] else 'unchecked'}.", "success")
    session.modified = True
    return redirect(url_for('show_list', list_id=list_id))
...
def mark_all_todos_completed(lst, list_id):
    ...
    flash("All todos have been marked complete.", "success")
    session.modified = True
    return redirect(url_for('show_list', list_id=list_id))
